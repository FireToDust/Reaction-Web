...
function setArray2DRange(
    ...
    if (dataWidth + dataOffsetX > targetWidth || dataHeight + dataOffsetY > targetHeight) {
        throw new RuleError("2D Range out of bounds");
    }
    if (target.length !== targetWidth * targetHeight || data.length !== dataWidth * dataHeight) {
        throw new RuleError("Data length does not match data width * data height");
    }
...
export class Rules{
     ...
     */
    public add_reaction(tile_type: number, reaction: Reaction): this {
        if (tile_type >= MAX_TILE_TYPES) {
             throw new RuleError(`Tile type out of range: ${tile_type}. Maximum is ${MAX_TILE_TYPES - 1}`);
        }
        const index = this.reaction_tracker[tile_type]!;
        if (index >= MAX_REACTIONS_PER_TYPE) {
            throw new RuleError(`Too many reactions for tile type ${tile_type}. Maximum is ${MAX_REACTIONS_PER_TYPE}`);
        }
        ...
        const dataOffsetY = index * 16;
         if (dataOffsetX + 4 > this.width || dataOffsetY + 16 > this.height) {
             throw new RuleError(`Reaction data for tile ${tile_type}, reaction ${index} would be out of bounds.`);
         }
...
export class Reaction {
     ...
     */
    public add_condition(condition: Condition, value: number): this {
        if (this.condition_number >= MAX_CONDITIONS_PER_REACTION) {
            throw new RuleError(`Too many conditions added to this reaction. Maximum is ${MAX_CONDITIONS_PER_REACTION}`);
        }
        ...
        if (this.condition_number == 0) {
            ...
            const originalCondData0 = this.data[targetIndexX0]!;
...
function datifyConditionTree(conditionTree: Condition, value: number): Uint32Array {
    const nodes = [conditionTree];
    const tileData: number[] = []; // Stores the packed data for LEAF nodes 0-8
    let conditionTypes = 0; // Packed modes for INTERNAL nodes 0-6

   

    // Build the tree level by level up to node 6 (7 internal nodes)
    for (let i = 0; i < 7; i++) { // Loop for internal nodes 0..6
        const node = nodes[i]!; // Get the node to process for this level
        ...
        if (node instanceof COperator){
            ...
            throw new RuleError("Unexpected node type during condition tree processing.");
        }
    }
    ...
    for (let i = 7; i < nodes.length && tileData.length < 8; i++) { // Process nodes intended as leaves
        const node = nodes[i]!;
    ...
    for (let i = 0; i < tileData.length; i++) { // Should loop 8 times for leaves 0-7
        const dataIndex = 1 + Math.floor(i / 3); // Target element: 1, 2, or 3
        const indexInChunk = i % 3;              // Index within the chunk: 0, 1, or 2
        const bitOffset = indexInChunk * 9;      // Offset: 0, 9, or 18 (Shader expects LSB first)
    
        const leafData = tileData[i]! & 0b111111111; // Mask the 9-bit leaf data
    
        // Shift the NEW leaf data to its correct position and OR it in
        data[dataIndex]! |= (leafData << bitOffset);
    }
    return data;
}
...
export enum Tile {
    ...
    GRASS = 4,
    MUD = 5,
    SAND = 20, // TODO: This should be 6, but keeping 20 for backward compatibility
}
...
export function and(...elements: Condition[]): Condition {
    if (elements.length < 1) {
        throw new RuleError("Cannot take the and of nothing.");
    }
    if (elements.length == 1) {
        return elements[0]!;
    }
...
export function or(...elements: Condition[]): Condition {
    if (elements.length < 1) {
        throw new RuleError("Cannot take the or of nothing.");
    }
    if (elements.length == 1) {
        return elements[0]!;
    }
...
export function not(element: Condition): Condition {
    if (element instanceof COperator) {
        switch (element.type) {
            case 0:
                return new NandNode(element.first, element.second);
            case 1:
                return new NorNode(element.first, element.second);
            case 2:
                return new OrNode(element.first, element.second);
            case 3:
                return new AndNode(element.first, element.second);
            default:
                throw new RuleError("COperator with invalid type");
        }
    } else if (element instanceof TileAt) {
        return new NandNode(element, element);
    } else {
        throw new RuleError("Cannot negate unknown condition type");
    }
}
